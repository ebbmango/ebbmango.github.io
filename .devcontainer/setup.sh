...
if [[ -n "${ICONS_JSON_B64:-}" ]]; then
	...
	trap 'rm -f "$temporary_icons"' EXIT
	...
	if ! node -e 'JSON.parse(require("node:fs").readFileSync(process.argv[1], "utf8"))' "$temporary_icons"; then
		echo "Error: ICONS_JSON_B64 did not decode to valid JSON." >&2
		exit 1
	fi
	...
	echo "The site may not build or run until that Codespaces secret is added and the container is rebuilt." >&2
fi
